...
p_dens_hier <- mcmc_dens(draws_df, pars = vars(mu_lambda, sigma_lambda)) +
    labs(
        title = "Posterior Distributions for Hierarchical Parameters",
...
p_dens_alpha <- mcmc_dens(draws_df, pars = vars(alpha)) +
    labs(title = "Posterior Distribution for Weibull Shape (alpha)")

# Combine some plots
combined_dens_plots <- (p_dens_betas) / (p_dens_hier + p_dens_alpha) + plot_layout(heights = c(2, 1))

ggsave("posterior_densities.png", combined_dens_plots, width = 8, height = 8, dpi = 300)
print("Posterior density plots saved to posterior_densities.png")

# Visualize Region-specific baseline scales (lambda)
# Need region mapping
region_map <- readRDS("processed_water_survival_data.rds") %>%
    distinct(country, region) %>%
    left_join(data.frame(
        country = unique(readRDS("processed_water_survival_data.rds")$country),
        country_idx = 1:length(unique(readRDS("processed_water_survival_data.rds")$country))
    ), by = "country") %>%
    left_join(data.frame(
        region = levels(readRDS("processed_water_survival_data.rds")$region),
        region_idx = 1:length(levels(readRDS("processed_water_survival_data.rds")$region))
    ), by = "region") %>%
...
ggsave("posterior_lambda_regions.png", p_lambda_regions, width = 8, height = 6, dpi = 300)
print("Region-specific lambda plots saved to posterior_lambda_regions.png")
...
ggsave("mcmc_trace_plots.png", p_trace, width = 10, height = 8, dpi = 300)
print("Trace plots saved to mcmc_trace_plots.png")
...
ggsave("mcmc_convergence_plots.png", combined_diag_plots, width = 10, height = 5, dpi = 300)
print("Convergence diagnostic plots saved to mcmc_convergence_plots.png")
...
obs_data <- readRDS("processed_water_survival_data.rds")
...
ggsave("ppc_total_events.png", p_ppc_total_events, width = 6, height = 5, dpi = 300)
print("PPC plot for total events saved to ppc_total_events.png")
...
ggsave("ppc_events_region.png", p_ppc_events_region, width = 8, height = 6, dpi = 300)
print("PPC plot for events per region saved to ppc_events_region.png")
